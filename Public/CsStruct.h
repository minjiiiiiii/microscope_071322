...
#pragma pack (8)   //struct단위를 8-byte로 설정 
...
typedef struct
{
	...
	void *	pDataBuffer;		//!< Pointer to application-allocated buffer to hold requested data.  For 8-bit CompuScope models,
	                            ...
	                            //!< input CompuScope models in 32-bit input mode, data values are formatted as uInt32.
								 ...
								 //void pointer -> 모든 자료형의 데이터 가리킬 수 있음. 역참조 하기 위해서는 형변환 필요 
	//! Notification event
...
typedef struct
{
	...
	int64	i64ActualStart;		//!< Address relative to the trigger address of the first sample in the buffer / buffer에 있는 첫번째 샘플의 trigger address로 부터의 상대적 주소 
